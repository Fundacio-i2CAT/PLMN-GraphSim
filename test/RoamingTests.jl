...
#     session (TS 23.122; TS 23.502 §4.2.2.2.2/§4.3.2.2.2). σ ≈ 3250 B, the flow
...
#     handover (~N2 1150 B + N32 envelope ≈ 1300 B), session survives.
# 6G-RUPA: enrollment (~200) + flat renumber (200) + internetwork-DIF
# advertisement (~50) ≈ 450 B; the flow survives in BOTH semantics (EFCP keyed on
...
@testset "Roaming (§7.4)" begin
    ...
    @testset "6G-RUPA border crossing: enrollment + renumber, flow survives" begin
        ...
        @test state.sigma_roam_rupa == 450
    ...
    @testset "roamer injection end-to-end (mobile lifecycle)" begin
        ...
        @test st1.sigma_roam_5g == 5 * 3250
        @test st1.sigma_roam_rupa == 5 * 450
