...
SHP_PATH = os.path.join(AGENTS_DIR, 'cb_2024_us_county_500k', 'cb_2024_us_county_500k.shp')
...
def process_data():
    ...
    if pop_col not in df.columns:
        ...
        possible_cols = ['POPESTIMATE2024', 'POPESTIMATE2023', 'POPESTIMATE2022', 'POPESTIMATE2020']
    ...
    df_subset.rename(columns={pop_col: 'population', 'CTYNAME': 'name', 'STNAME': 'state_name'}, inplace=True)
    ...
    # Fill NaN population with 0 or drop? Let's fill with 0 for now to avoid breaking things
    ...
    # Some shapefiles have 'NAME' column which is the county name. 
    # Our CSV merge brought in 'name' (from CTYNAME). 
    # Let's prioritize the CSV name but keep the shapefile one if needed.
    ...
    # Calculate centroids
    # Warning: Centroid of a polygon on a sphere (lat/lon) is an approximation if done in 4326, 
    # but usually acceptable for this scale. For better accuracy, project to equal area, calculate, then reproject.
    # For simplicity here, we do it in 4326 or the original CRS if it was projected.
    # Actually, let's use the original geometry for centroid if it was projected, but it likely wasn't.
